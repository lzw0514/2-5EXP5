...
import cn.edu.xmu.javaee.productdemoaop.mapper.jpa.ProductRepository;
...
public class ProductDao {
    ...
    @Autowired
    public ProductDao(ProductPoMapper productPoMapper, OnSaleDao onSaleDao, ProductAllMapper productAllMapper , ProductRepository productRepository) {
    ...
    public List<Product> findProductByName_jpa(String name) throws BusinessException {
        List<Product> productList = new ArrayList<>();
        List<ProductEntity> product_Entities = productRepository.findByName(name);

        product_Entities.forEach((mainProductEntity -> {
                    Product mainProduct = CloneFactory.copy(new Product(), mainProductEntity);
                    List<OnSale> latestOnSale = productRepository.findCurrentOnSalesByProductId(mainProduct.getId()).stream().map(entity -> CloneFactory.copy(new OnSale(), entity))
                    .collect(Collectors.toList());
                    List<Product> otherProducts = productRepository.findOtherProductsByGoodsId(
                                    mainProductEntity.getGoodsId(), mainProductEntity.getId()).stream()
                            .map(entity -> CloneFactory.copy(new Product(), entity))
                            .collect(Collectors.toList());
                    mainProduct.setOtherProduct(otherProducts);
                    mainProduct.setOnSaleList(latestOnSale);
